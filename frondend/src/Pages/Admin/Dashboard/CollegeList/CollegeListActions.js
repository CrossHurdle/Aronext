...
import CollegeRejectModal from "./CollegeRejectModal";

const btnStyles = { fontSize: "0.7rem" };

const CollegeListActions = ({ record }) => {
    ...
    return (
        <>
            <CollegeListModal id={record?.id} show={modalOpen} handleClose={() => setModalOpen(false)} />
            <CollegeRejectModal id={record?.id} show={rejectModalOpen} handleClose={() => setRejectModalOpen(false)} handleReject={() => setRejected(true)} />
...
export default CollegeListActions;