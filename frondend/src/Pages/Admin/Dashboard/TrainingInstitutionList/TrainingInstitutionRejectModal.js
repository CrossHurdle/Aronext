...
const TrainingInstitutionRejectModal = ({ show, handleClose, handleReject, id }) => {
    const [rejectReason, setRejectReason] = useState("");

    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            const result = { status: "2", rejectedReasonByAdmin: JSON.stringify(rejectReason || "") };
            if (result.data) {
                handleReject();
                handleClose();
            }
    ...
    return (
        <Modal show={show} onHide={handleClose} centered>
            <Modal.Header closeButton>
                <Modal.Title>Reject TrainingInstitution</Modal.Title>
            </Modal.Header>
...
export default TrainingInstitutionRejectModal;