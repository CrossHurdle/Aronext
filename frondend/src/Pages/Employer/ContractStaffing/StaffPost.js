...
export default function StaffPost() {
  ...
  const selectworklocation = [
    { label: "Work from client office", value: "Work from client office" },
  ...
  const handleFormSubmit = async () => {
    const StaffData = {
      ...
      Qualification : getValues().worklocation,
    ...
    <div>
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <div className="personal-informations p-4">
          <div className="staff-title mb-5">Post your Staffs here</div>
          <Row className="mb-3">
            ...
            <Col xs={24} sm={24} md={10} className="px-4">
              <label>Additional Technology</label>
              <MultiSelect
                value={additionaltechnology}
                options={selectAdditionalTechnology}
                className="seminarSelect"
                {...register("additionaltechnology", {
                  required: true,
                  onChange: (e) => {
                    setAdditionalTechnology(e.target.value);
                  },
                })}
              ></MultiSelect>
            </Col>
          ...
          <Row className="mt-5">
          <Col sm={24} md={10} className="Stud-personal-col px-4 ">
              <label className="input-title">Number of Candidates</label>
              <input
                className="seminarInput"
                type="text"
                {...register("Candidates", { required: true })}
              />
              {errors.Candidates && (
                <span className="Stud-personal-error">Count required</span>
              )}
            </Col>
            <Col sm={24} md={10} className="Stud-personal-col px-4 ">
          ...
          <Row className="mt-5">
            ...
            <Col xs={24} sm={24} md={10} className="price_field px-4">
              <div >
              ...
              <Checkbox onClick={toogleDisable} className="checkbox_field ms-2">
                {!disabled ? "Disclose Price" : "Disclosed " }
